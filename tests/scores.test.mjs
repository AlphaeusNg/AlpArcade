...
const statScores = createScores().scores;
assert.equal(statScores.submitScore("snake", 48, { eaten: 9, level: 4 }).isHighScore, true);
assert.equal(statScores.getState().highScores.snake.eaten, 9);
assert.equal(statScores.getState().highScores.snake.level, 4);
assert.equal(statScores.getState().highScores.snake.eatenTotal, 9);
assert.equal(statScores.formatBest("snake", statScores.getState().highScores.snake), "9 eaten");
assert.equal(statScores.submitScore("snake", 20, { eaten: 4, level: 2 }).isHighScore, false);
assert.equal(statScores.getState().highScores.snake.eatenTotal, 13);
assert.equal(statScores.formatRun("shooter", 400, { stats: { wave: 7 } }), "Wave 7");
assert.equal(statScores.formatRun("tictactoe", 0, { stats: { wins: 12 } }), "12 wins");
assert.equal(statScores.submitScore("shooter", 220, { wave: 5 }).isHighScore, true);
assert.equal(statScores.getState().highScores.shooter.wave, 5);
assert.equal(statScores.formatBest("shooter", statScores.getState().highScores.shooter), "Wave 5");
const summary = statScores.summarizeRun("breaker", 80, { row: 6 });
assert.equal(summary.headline, 6);
assert.equal(summary.headlineKey, "row");
assert.equal(summary.stats.row, 6);
assert.equal(summary.stats.score, 80);
